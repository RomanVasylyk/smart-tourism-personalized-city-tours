...
from conftest import FakeConnection, FakeRoutingService, MixedRoutingService, SlowWalkingRoutingService


def test_generate_route_returns_planned_stop(client, monkeypatch):
    rows = [
        {
            "id": 10,
            "name": "Nitra Castle",
...
def test_generate_route_leg_returns_direct_leg(client, monkeypatch):
    monkeypatch.setattr("app.services.route_planner.get_routing_service", lambda: FakeRoutingService())
...
def test_generate_route_can_return_transit_segments(client, monkeypatch):
    rows = [
        {
            "id": 11,
            "name": "Pyramida",
...
def test_generate_route_falls_back_to_walking_when_transport_graph_is_missing(client, monkeypatch):
    rows = [
        {
            "id": 13,
...
def test_generate_route_prefers_exact_trip_times_when_available(client, monkeypatch):
    rows = [
        {
            "id": 12,
            "name": "Pyramida",
